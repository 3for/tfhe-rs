...
impl ServerKey {
    ...
    /// i.e: [A, B, C] -> [A + B + C, B, C]
    /// and returns the inclusive range indicating the range of blocks which where addition where
            ...
            chunks_iter
                .map(|chunk| {
                    ...
                    rayon::join(
                        ...
                        || {
                            // Contrary to non overflowing version we allways extract all carries
    ...
    /// Returns a boolean indicating of the sum overtlowed, that is,
