...
fn generate_accumulator_many_lut<Scalar: UnsignedTorus + CastFrom<usize>>(
    ...
    assert!(fn_count <= (total_plaintext_modulus / 2));

    // Create the accumulator
    let mut accumulator_scalar = vec![Scalar::ZERO; polynomial_size.0];

    let boxes = accumulator_scalar.chunks_exact_mut(box_size * fn_count);

    // Fill each box with the encoded denoised value
    for (msg_value, boxes) in boxes.enumerate() {
        let msg_value = Scalar::cast_from(msg_value);
        let boxes = boxes.split_into(fn_count);

        for (box_, func) in boxes.into_iter().zip(funcs.iter()) {
            let func_eval = func(msg_value) * msg_delta;
            box_.fill(func_eval);
        }
    }
    ...
    ClassicTestParams<Scalar>: KeyCacheAccess<Keys = ClassicBootstrapKeys<Scalar>>,
{
    ...
    for fn_count in 1..=funcs.len() {
        ...
        let msg_delta: Scalar = encoding_with_padding / msg_modulus;
        let mut msg = msg_modulus;
        ...
        let lut_delta: Scalar = encoding_with_padding / total_plaintext_modulus;

        // The box size is required to know where to look for the evaluations of the other functions
        let (accumulator, box_size) = generate_accumulator_many_lut(
            polynomial_size,
            glwe_dimension.to_glwe_size(),
        ...
        while msg != Scalar::ZERO {
            ...
            for _ in 0..NB_TESTS {
                let plaintext = Plaintext(msg * msg_delta);
                ...
                for (fn_idx, func) in funcs.iter().enumerate() {
                    extract_lwe_sample_from_glwe_ciphertext(
                        &tmp_acc,
                        &mut out_pbs_ct,
                        MonomialDegree(fn_idx * box_size),
                    );
                    ...
                    let decoded = round_decode(decrypted.0, lut_delta) % total_plaintext_modulus;
